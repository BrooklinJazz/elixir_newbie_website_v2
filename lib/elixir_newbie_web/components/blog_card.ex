defmodule ElixirNewbieWeb.Components.BlogCard do
  ...
  def blog_card(assigns) do
    ~H"""
      <article>
        <figure class="overflow-hidden h-90 rounded-3xl aspect-[2/1] transition duration-500 ease-in-out ring ring-white hover:ring-offset-2">
          <img class="w-auto rounded-lg " src={static_url(ElixirNewbieWeb.Endpoint, "/images/posts/#{@blog.cover_image}")}/>
        </figure>
        <p class="text-2xl text-white 4k:text-4xl lg:text-2xl"><%= @blog.title %></p>
