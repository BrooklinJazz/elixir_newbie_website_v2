defmodule ElixirNewbieWeb.PodcastLive do
  ...
  def render(assigns) do
    ~H"""
    <section class="bg-[url('/images/PodcastBackground1.png')] bg-top min-h-screen bg-repeat pb-96">
      <div class="w-full h-full bg-black/50">
        <.navigation/>
        <section class="w-3/4 m-auto text-white">
          ...
          <article class="flex items-center justify-center sm:space-x-12">
            ...
            <figure class="w-3/4 sm:w-2/5 lg:w-1/4 text-xl">
        ...
        <section class="podcast-description bg-purple-500/50 mx-auto mt-12 w-3/4 rounded-xl p-6 text-white text-white">
          <%= raw @current_episode.description %>
        </section>
        <section class="w-3/4 mx-auto mt-10 grid grid-cols-4 text-white">
  ...
  def audio_player(assigns) do
    ...
    <article class="m-6 flex justify-center">
      <div id="player-progress" phx-hook="Progress" class="h-3 rounded-xl bg-gray-100 hover:cursor-pointer w-2/3 lg:w-2/3">
        <div style={"width: #{max(progress_percent(@playing, @player_time, @current_episode.seconds), 1)}%"} class="h-full rounded-l-full bg-gray-400 w-10" />
      </div>
    </article>
    <article class="grid grid-cols-12 md:grid-rows-2 lg:grid-rows-1">
      ...
      <p id="player-time" class="col-start-2 md:flex hidden h-12 items-center justify-center">
        <%= Formatter.to_hh_mm_ss(@player_time) %>/<%= @current_episode.hh_mm_ss %>
      </p>
      <a href={@current_episode.audio_url} target="_blank" download class="col-start-4 sm:flex hidden h-12 items-center justify-center" aria-label="Download Podcast Episode"><Heroicons.arrow_down_tray class="h-6 w-6"/></a>
  ...
  def episode_button(assigns) do
    ...
    <.link phx-hook="ScrollToTop" id={"podcast-episode-#{@episode.episode_number}"} patch={~p"/podcast?episode=#{@episode.episode_number}"}>
      <figure class="relative h-24 bg-black/80 mt-6 rounded-3xl ring ring-white transition duration-500 ease-in-out hover:ring-offset-2">
